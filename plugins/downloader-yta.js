...
let sedangMendownload = false;

let handler = async (m, { conn, args, usedPrefix, command }) => {
    if (sedangMendownload) return m.reply("Tunggu proses sebelumnya selesai...");

    sedangMendownload = true;
    if (!args[0]) return m.reply(`*• Contoh:* ${usedPrefix + command} https://youtu.be/xxxx`);

    await conn.reply(m.chat, wait, m);
    try {
        let Ytdl = await ytmp3(args[0]);
        let dls = "Kreybot PluginPair";
