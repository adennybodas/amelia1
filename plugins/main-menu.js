...
let menulist = async (m, { conn, usedPrefix, command, args }) => {
  ...
  if (perintah === 'tags') {
    ...
    let list = `Hi @${m.sender.split("@")[0]} 👋️ *Aku AMEL*
...
`
 const pp = await conn.profilePictureUrl(m.sender, 'image').catch((_) => "https://telegra.ph/foto-05-24-31");
...
{ text: `➳ *L O A D I N G .*`, timeout: 200 },
{ text: `➳ *L O A D I N G*`, timeout: 200 },
{ text: `➳ *W E L C O M E  T O  MY FAMILY ADM*`, timeout: 200 },
...
_Iam AMEL, How can I assist you today?_\n\n`
    + allTagsAndHelp
;
    const pp = await conn.profilePictureUrl(m.sender, 'image').catch((_) => "https://telegra.ph/foto-05-24-31");
if (global.menu === 'simple') {
